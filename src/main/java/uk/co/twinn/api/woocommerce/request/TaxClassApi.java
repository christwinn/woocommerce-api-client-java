...
public class TaxClassApi extends ApiRequest {
    ...
    public Creator<?> create(String name){

        return new Creator<>(name);

    }

    public Deleter<?> delete(String taxClassSlug, boolean force){

        return new Deleter<>(taxClassSlug, force);
    ...
    private TaxClassApi(Creator<?> creator){
    ...
    public static class Deleter<T extends Deleter<T>> extends CoreDeleterRequest.DeleterCoreStringKey<T>{
